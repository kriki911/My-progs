...
public class BackwardRepeated extends JFrame {
	int sizeX = 101;
	int sizeY = 101;
	int boxSize = 7;
	...
	public BackwardRepeated() {
		...
		end = myMaze.mazeArray[y][x];
		end.setEnd();
		start = myMaze.mazeArray[sizeY-1][sizeX-1];
		myMaze.startBox = start;
		start.setSart();
		calculateH(myMaze);
//		path = searchMaze(myMaze, start);
	...
	public static void main(String[] args) {
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				BackwardRepeated mainMaze = new BackwardRepeated();
...
//			}
		...
		System.out.println("this is # of expanded: " + xx);
		return pathA;
	}
			...
			//	System.out.println(closedList);
				...
				for(int i = 0; i < currentWalls.size(); i++) {
					...
					if(!closedList.contains(neighbor)) {
						...
						int currentToStart = currentPointer.h;//calculateDistanceToStart(currentPointer);
						...
						if(totalDistance <= neighbor.f || !openList.contains(neighbor) ) {
							...
							if(neighbor.h > currentPointer.h + 1) {
								neighbor.h = currentPointer.h + 1;
	...
	public void calculatePath(Box currentPos) {
		...
		int yMoves = Math.abs(i- end.getY());
		int xMoves = Math.abs(i- end.getX());
	...
	public void resetParents() {
		...
		for(int i = 0; i < myMaze.rows; i++ ) {			//rows are going RIGHT/LEFT
			for(int j = 0; j < myMaze.columns; j++) { 	//columns are going DOWN
				Box tempBOX = myMaze.mazeArray[i][j];
				tempBOX.g = Math.abs(start.y - tempBOX.y) + Math.abs(start.x - tempBOX.x);
				tempBOX.search = 0;
			}
		}
	}
